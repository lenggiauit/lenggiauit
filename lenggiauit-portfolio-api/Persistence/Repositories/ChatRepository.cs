...
namespace Lenggiauit.API.Persistence.Repositories
{
    public class ChatRepository : BaseRepository, IChatRepository
    {
        ...
        public ChatRepository(LenggiauitContext context, ILogger<ChatRepository> logger) : base(context)
        {
            _logger = logger;
        }
        ...
        public async Task<List<Conversation>> GetConversationListByUser(Guid userId, BaseRequest<GetConversationListRequest> request, AppSettings appSettings)
        {
            try
            {
                ...
                if(list.Count == 0)
                {
                    var naUser = _context.User.Where(u => u.Email.Equals("lenggiauit.vn@gmail.com")).FirstOrDefault();
                    if (naUser != null)
                    {
